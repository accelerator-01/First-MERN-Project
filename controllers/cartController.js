...
import { findOne as _findOne } from "../models/Item";
...
export async function add_cart_item(req, res) {
  const userId = req.params.id;
  const { productId, quantity } = req.body;

  try {
    let cart = await findOne({ userId });
    let item = await _findOne({ _id: productId });
    if (!item) {
...
export async function update_cart_item(req, res) {
  const userId = req.params.id;
